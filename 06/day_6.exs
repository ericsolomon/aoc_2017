defmodule Day6 do
  ...
  def solve(input, seen, acc) do
    ...
    new_input =
      distribute_blocks(
        List.replace_at(input, largest_index, 0),
        length(input),
        largest_index + 1,
        Enum.at(input, largest_index)
      )

    if Enum.member?(seen, new_input),
      do: {new_input, acc},
      else: solve(new_input, seen ++ [new_input], acc + 1)
  end
  ...
  defp distribute_blocks(input, length, index, blocks) do
    distribute_blocks(
      List.update_at(input, rem(index, length), &(&1 + 1)),
      length,
      index + 1,
      blocks - 1
    )
